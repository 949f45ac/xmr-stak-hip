...
#define WAIT_FOR(x, n) asm volatile("s_waitcnt vmcnt(" #n ")": : "v" (x) : "memory"); FENCE(x)
#else
...
__global__ void cryptonight_core_gpu_phase2( int threads, int bfactor, int partidx, uint64_t * __restrict__ d_long_state_64, uint32_t * __restrict__ d_ctx_a, uint32_t * __restrict__ d_ctx_b, uint32_t * __restrict__ d_ctx_state, uint32_t startNonce, uint32_t * __restrict__ d_input )
{
	...
	for ( i = start; i < end; ++i )
	{
		...
		for ( int x = 0; x < 2; ++x )
		{
			...
			ulonglong2 y2 = d_xored;
			y2.x *= same_adr;
			y2.y *= same_adr;

			
			WAIT_FOR(ldst0, 2);
			y2.x += (1-same_adr) * ldst0;
			RETIRE(ldst0);
...
extern "C" void cryptonight_core_cpu_hash(nvid_ctx* ctx, uint32_t nonce)
{
	...
	{
		hipLaunchKernelGGL(cryptonight_core_gpu_phase1, dim3(grid), dim3(block8), 0, 0, ctx->device_blocks*ctx->device_threads,
			bfactorOneThree, i,
	...
	for ( int i = 0; i < partcountOneThree; i++ )
	{
		hipLaunchKernelGGL(cryptonight_core_gpu_phase3, dim3(grid), dim3(block8), 0, 0, ctx->device_blocks*ctx->device_threads,
			bfactorOneThree, i,
			ctx->d_long_state,
